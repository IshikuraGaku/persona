...
#python train.py PNewInput.en PNewOutput.en vocab.en personaVocab.en --validation-source PTestInput.en --validation-target PTestOutput.en --unit 300 --epoch 20 --batch 16
import loadData
import personaModel
import argparse
...
def main():
    ...
    train_data, vocabulary, peVocabulary,  _, _ = loadData.makeData(args.SOURCE, args.TARGET, args.VOCAB, args.PERSONA_VOCAB)#np
    ids_words = loadData.ids_word(vocabulary)
    ...
    if args.validation_source and args.validation_target:
        test_data, _, _, _, _ = loadData.makeData(args.validation_source, args.validation_target, args.VOCAB, args.PERSONA_VOCAB)
        ...
        def translate(trainer):
            ...
            if args.gpu >= 0:
                ...
                source = source[1]
            print("source\n"+' '.join([ids_words.get(int(y), UNK) for y in source]))
            source = F.pad_sequence([source], 50, -1)
            result = model.predict(source.data, target[0])
            #あってんのか？
            result = ' '.join([ids_words.get(int(y), UNK) for y in result[0]])#ここは(1,50)しかないはず？
            print('\nresult')
            print(result)
            print("\ntrue\n"+' '.join([ids_words.get(int(y), UNK) for y in target[1]]))
...
class CalculateBleu(chainer.training.Extension):
    ...
    def __call__(self, trainer):
        with chainer.no_backprop_mode():
            ...
            sources = F.pad_sequence(sources, 50, -1)
            targets = F.pad_sequence(targets, 50, -1)
