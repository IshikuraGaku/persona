...
class Encoder(chainer.Chain):
    def __init__(self, n_hidden):
        ...
        with self.init_scope():
            ...
            self.lstm_backward3 = L.StatelessLSTM(n_hidden, n_hidden)#wordとpersona
...
class Decoder(chainer.Chain):
    ...
    def __call__(self, firstEOS, y, pey, c_input, h_input):
        ...
        if self.decoderCount <= 100000: #安定するまで正解データを突っ込む
            ...
            for yi in y:            
                ...
                firstEOS = loadData.sequence_embed(self.embed, yi.data)#memory削減
            ...
            for _ in range(max_len):            
                ...
                firstEOS = loadData.sequence_embed(self.embed, pyid)
                firstEOS = F.concat((firstEOS, pey), axis=1)
    ...
    def predict(self, firstEOS, targetPersona, c_input, h_input):
        ...
        for _ in range(50):
            ...
            firstEOS = loadData.sequence_embed(self.embed, py)
            firstEOS = F.concat((firstEOS, targetPersona), axis=1)
            c_old = c_new
            h_old = c_new
        return output_word
...
class Model(chainer.Chain):
    def __init__(self, dic_vocab, dic_personaVocab, n_hidden, batch, device):
        super(Model, self).__init__()
        with self.init_scope():
            self.emb = L.EmbedID(len(dic_vocab), n_hidden, ignore_label=-1)
            self.personaEmb = L.EmbedID(len(dic_personaVocab), n_hidden, ignore_label=-1)
            self.lineCED = L.Linear(n_hidden*2, n_hidden)
            self.lineHED = L.Linear(n_hidden*2, n_hidden)
            self.enc = Encoder(n_hidden)
            self.dec = Decoder(len(dic_vocab), n_hidden, self.emb)
            self.con = Context(n_hidden)

        self.n_hidden = n_hidden
        self.batch = batch
        self.n_vocab = len(dic_vocab)
        self.n_personaVocab = len(dic_personaVocab) 
        self.dic_vocab = dic_vocab
        self.dic_personaVocab = dic_personaVocab
        self.device = device
    ...
    def __call__(self, ex, ey, pex, pey):
        ...
        perp = self.xp.exp(showLoss * self.batch / 50)
    ...
    def personaEmbed(self, pey):
        noneNumber = self.dic_personaVocab.get("none")
